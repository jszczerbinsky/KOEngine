...
static struct ConsoleHookNamespace *findConsoleNamespace(const char *name)
{
...
static void updateConsoleLinesTex()
{
  const SDL_Color foreground = {.r = 255, .g = 255, .b = 255};
  ...
  for (int i = 0; i < CONSOLE_LINES_MAX; i++)
  {
    ...
    SDL_Surface *line = TTF_RenderText_Solid(
        consoleFont, strlen(consoleLineBuff) == 0 ? " " : consoleLineBuff, foreground);
    consoleLines[i].tex  = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888,
                                             SDL_TEXTUREACCESS_TARGET, CONSOLE_WIDTH, line->h);
    ...
    SDL_Rect dest = {.x = 0, .y = 0, .w = line->w, .h = line->h};
...
static void consolePrintHelp(struct ConsoleHookNamespace *namespace)
{
  if (namespace)
  {
    ...
    while (currentHook)
    {
      if (currentHook->namespace == namespace)
      {
        ...
        snprintf(str, CONSOLE_LINE_LENGTH, "  %s    %s.%s", typeStr, currentHook->namespace->name,
                 currentHook->name);
...
static void consoleExecute(const char *cmd)
{
  if ((strlen(cmd) == 4 && strncmp(cmd, "HELP", 4) == 0) ||
      (strlen(cmd) > 4 && strncmp(cmd, "HELP ", 5) == 0))
  ...
  else
  {
    ...
    if (*ptr != '\0')
    {
      ...
      if (namespace)
      {
        ...
        {
          if (currentHook->namespace == namespace &&
              strncmp(currentHook->name, cmd + namespaceLength + 1, wordLength) == 0)
          {
            ...
            else
            {
              ...
              else
              {
                char str[CONSOLE_LINE_LENGTH];
                if (currentHook->type == CONSOLEHOOK_INT)
                  snprintf(str, CONSOLE_LINE_LENGTH, "%s = %d", currentHook->name,
                           *((int *)currentHook->value));
                else
                  snprintf(str, CONSOLE_LINE_LENGTH, "%s = %f", currentHook->name,
                           *((float *)currentHook->value));
                PrintConsoleLine(str);
              }
            }
            return;
...
void consolePutChar(char c)
{
  ...
  {
    case 8:
      if (length >= 1) consoleLines[0].str[length - 1] = '\0';
      break;
    case '\n':
      for (int i = CONSOLE_LINES_MAX - 1; i > 0; i--)
        strcpy(consoleLines[i].str, consoleLines[i - 1].str);
...
void OpenConsole() { ConsoleActive = 1; }

void CloseConsole() { ConsoleActive = 0; }
