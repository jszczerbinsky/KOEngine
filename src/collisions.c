...
bool checkCollision(GameObject *ent1, GameObject *ent2)
{
  ...
  if (ent1->onCollision) (*ent1->onCollision)(ent1, ent2, &ignoreCollision);

  if (ent2->onCollision) (*ent2->onCollision)(ent2, ent1, &ignoreCollision);

  if (ent1->colliderMode == COLLIDER_MODE_EVENTS_ONLY ||
      ent2->colliderMode == COLLIDER_MODE_EVENTS_ONLY || ignoreCollision)
    return false;
