...
NetworkClient *addClient(struct sockaddr *addr)
{
  ...
  if (ClientList != NULL) ClientList->prev = c;
...
static int hostTask(void *threadid)
{
  ...
  while (1)
  {
    ...
    bytes = recvfrom(udpSocket, buff, NETWORK_DATAGRAM_MAX_BYTES, 0, (struct sockaddr *)&addr,
                     &addrlen);
    if (exitThread)
    {
      killSocket();
    ...
    if (bytes > 0)
    {
        ...
        if (*DATAGRAM_FLAGS(buff) == NETWORK_FLAG_CONNECT)
        {
          ...
          if (accept)
          {
            ...
            *DATAGRAM_FLAGS(buff) = NETWORK_FLAG_ACCEPTED;
            ...
            if (HOST_SETTINGS->onConnection) (*HOST_SETTINGS->onConnection)(newC);
      ...
      else
      {
        ...
        {
          if (HOST_SETTINGS->onDisconnection) (*HOST_SETTINGS->onDisconnection)(matchedClient);
    ...
    {
      next = c->next;
      if (HOST_SETTINGS->clientTimeoutTime != TIMEOUT_DISABLE &&
          now - c->lastDatagram > HOST_SETTINGS->clientTimeoutTime)
      {
        Log("Client timed out");

        if (HOST_SETTINGS->onDisconnection) (*HOST_SETTINGS->onDisconnection)(c);
...
void CloseServer()
{
  ...
  while (ClientList) removeClient(ClientList);
}
