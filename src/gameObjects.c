...
static int killingAllGameObjects = 0;
...
void initGameObjects(unsigned int layers, unsigned int lightL)
{
  ...
  for (unsigned char layer = 0; layer < layersCount; layer++) objList[layer] = NULL;
}

void freeGameObjects() { free(objList); }
...
void inheritPosition(GameObject *obj, Vector2D *posPtr)
{
  ...
  posPtr->x = (cosTheta * (posPtrBackup.x - nonRotatedPosition.x)) -
              (sinTheta * (posPtrBackup.y - nonRotatedPosition.y)) + nonRotatedPosition.x;

  posPtr->y = (sinTheta * (posPtrBackup.x - nonRotatedPosition.x)) +
              (cosTheta * (posPtrBackup.y - nonRotatedPosition.y)) + nonRotatedPosition.y;
...
bool CheckAnyCollision(GameObject *obj)
{
  ...
  for (unsigned char layer = 0; layer < layersCount; layer++)
    for (GameObject *e = objList[layer]; e; e = next)
    {
      ...
      if (checkCollision(e, obj)) return true;
    }
...
void MoveLocal(GameObject *obj, float x, float y)
{
  ...
  Move(obj, cos(rads) * x + sin(rads) * y, sin(rads) * x - cos(rads) * y);
}
...
void addGameObject(GameObject *obj, unsigned int layer)
{
  ...
  if (objList[layer] != NULL) objList[layer]->prev = obj;
...
void removeGameObject(GameObject *gameObject)
{
  ...
  if (gameObject->next != NULL) gameObject->next->prev = gameObject->prev;
}
...
void SortLayer(unsigned int layer, int (*compare)(GameObject *, GameObject *))
{
  ...
  while (objList[layer])
  {
    ...
    while (current->next)
    {
      ...
      if ((*compare)(smallest, current)) smallest = current;
    }

    removeGameObject(smallest);
    smallest->prev = NULL;
    if (sorted) sorted->prev = smallest;
                            ...
                            unsigned int layer)
{
  ...
  obj->width            = w;
  obj->height           = h;
  obj->defaultTexture   = tex;
  obj->currentTexture   = tex;
...
void KillGameObject(GameObject *gameObject)
{
  ...
  {
    if (gameObject->ui->textTexture) SDL_DestroyTexture(gameObject->ui->textTexture);
    free(gameObject->ui);
  }

  if (gameObject->light != NULL) free(gameObject->light);

  removeGameObject(gameObject);

  if (gameObject->collider.vertices != NULL) free(gameObject->collider.vertices);
      ...
      if (e->active)
      {
        ...
        if (e->currentAnimation != NULL)
        {
          ...
          if (e->currentAnimation)
            e->currentTexture = e->currentAnimation->textures[(int)(e->animationCounter)];
        }

        renderGameObject(e, pos, camPos);

        if (e->loopCall != NULL) (*e->loopCall)(e);
      }
    }

    if (layer == lightLayer) renderLightTint();
  }
  if (ConsoleActive) renderConsole();
}
...
void KillAllGameObjects()
{
  ...
  for (unsigned char layer = 0; layer < layersCount; layer++)
    while (objList[layer] != NULL) KillGameObject(objList[layer]);
